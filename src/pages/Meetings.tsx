...
const Meetings: React.FC = () => {
  ...
  const [eventContractId, setEventContractId] = useState<string>("");
  ...
  const createEvent = async () => {
    ...
    try {
      ...
      setEventContractId(currentContractId);
      ...
      const tx = await (presenceEvents as any).create_event(
        {
          name: evtName,
          start_ts: BigInt(toEpoch(startLocal)),
          end_ts: BigInt(toEpoch(endLocal)),
          operator: address,
        },
        { publicKey: address },
      );
      ...
      if (need && need.length && need[0] && need[0] !== address) {
        addNotification(
          `Assine com ${need[0]} ou ative Supervisor no Admin`,
          "warning",
        );
      ...
      if (
        String(msg).includes("InvalidAction") ||
        String(msg).includes("UnreachableCodeReached")
      ) {
        addNotification(
          "Operador não autorizado no contrato de eventos. Ative Supervisor no painel do Admin.",
          "warning",
        );
      }
  ...
  const refreshList = useCallback(async () => {
    setLoading(true);
    logRef.current("info", "Buscando próximas reuniões");

    const currentContractId = await getEventContractId();
    if (!currentContractId) {
      logRef.current("error", "Contrato de eventos não configurado");
      notifyRef.current(
        "Contrato de eventos não configurado. Contate o administrador.",
        "warning",
      );
    ...
    setList(out);
    setLoading(false);
    logRef.current("success", `Próximas reuniões carregadas: ${out.length}`);
  }, []);
  ...
  const refreshClosed = useCallback(async () => {
    ...
    (presenceEvents as any).options.contractId = currentContractId;
    setEventContractId(currentContractId);
    const r = await (presenceEvents as any).list_closed({
      cursor: BigInt(closedCursor),
      limit: 10,
    });
    const arr: any[] = ((r as any)?.result as any[]) || [];
    const now = BigInt(Math.floor(Date.now() / 1000));
  ...
  const qrUrl = (eid: number) => {
    const cid =
      eventContractId && isValidContractId(eventContractId)
        ? eventContractId
        : "";
    const base = `${window.location.origin}/presence/${eid}`;
    ...
    <Layout.Content>
      <Layout.Inset>
        ...
        <div className="card" style={{ marginTop: 12 }}>
              ...
              <tbody>
                {list.map((ev) => (
                  <tr key={ev.id}>
                    ...
                    <td>
                      <span
                        className={`status ${ev.status === "Ativa" ? "status-on" : ev.status === "Encerrada" ? "status-off" : "status-pending"}`}
                      >
                        {ev.status}
                      </span>
                    </td>
                    <td style={{ display: "flex", gap: 8 }}>
                      ...
                      <Button
                        variant="secondary"
                        size="sm"
                        onClick={() => (window.location.href = qrUrl(ev.id))}
                      >
                        Presenças
                      </Button>
                      {isSupervisor && ev.status === "Ativa" && (
          ...
          {qrFor && (
            <div style={{ marginTop: 12 }}>
              <Text as="p" size="md">
                QR de presença para evento #{qrFor}
              </Text>
              <img
                alt="QR Code"
                style={{
                ...
                src={`https://api.qrserver.com/v1/create-qr-code/?size=240x240&data=${encodeURIComponent(qrUrl(qrFor))}`}
              />
            </div>
          )}
        </div>
        <div className="card" style={{ marginTop: 12 }}>
          ...
          >
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <Button
                ...
                size="sm"
                onClick={() => {
                  setClosedCursor(Math.max(0, closedCursor - 10));
                  void refreshClosed();
                }}
                disabled={closedLoading || closedCursor === 0}
              >
                Anterior
              </Button>
              <Button
                ...
                size="sm"
                onClick={() => {
                  setClosedCursor(closedCursor + 10);
                  void refreshClosed();
                }}
          ...
          {!closedLoading && closedList.length === 0 && (
            <Text as="p" size="md">
              Nenhuma encerrada
            </Text>
          )}
            ...
            >
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                <Input
                  ...
                  fieldSize="md"
                  placeholder="Ex: Reunião de P&D em Soroban"
                />
                <Input
                  label="Início"
                  id="start"
              ...
              <div className="confirmation-buttons">
                ...
                <button
                  ...
                  disabled={!address || !evtName || !startLocal || !endLocal}
                >
                  Criar
                </button>
              </div>
